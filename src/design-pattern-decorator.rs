...
fn main() {
    let message = Message {
        message: "Underlined message.".to_string(),
    };
    let underlined = UnderlinedMessage { decorated: message };
    println!("{}", underlined.print());

    let message = Message {
        message: "Indented message.".to_string(),
    };
    let indented = IndentedMessage { decorated: message };
    println!("{}", indented.print());

    let message = Message {
        message: "The first message is underlined then intended.".to_string(),
    };
    let underlined = UnderlinedMessage { decorated: message };
    let indented = IndentedMessage {
        decorated: underlined,
    };
    println!("{}", indented.print());

    let message2 = Message {
        message: "The second message is indented then underlined.".to_string(),
    };
    let indented2 = IndentedMessage {
        decorated: message2,
    };
    let underlined2 = UnderlinedMessage {
        decorated: indented2,
    };
